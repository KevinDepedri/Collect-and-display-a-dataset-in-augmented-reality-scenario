...
public class ARPlacementManager : DilmerGames.Core.Singletons.Singleton<ARPlacementManager>
{
    ...
    public string file_name = "coordinates.txt";
    private List<string> _file_names = new List<string>();
    ...
    private IEnumerator Screenshot()
    {
        ...
        var dirpath_xyztheta = Application.persistentDataPath + "/CaR/";
        ...
        if (!Directory.Exists(dirpath_xyztheta)) Directory.CreateDirectory(dirpath_xyztheta);


        File.WriteAllBytes(dirpath + "Image-" + (_retrievedPhotoCounter) + ".png", bytes);

        File.WriteAllBytes(dirpath_xyztheta + "Image-" + (_retrievedPhotoCounter) + _photoPosition.x + _photoPosition.y + _photoPosition.z + _photoRotation + ".png", bytes);

        //NEW
        /*
        _file_names.Add(final_path);

        File.WriteAllBytes(dirpath + "Image-" + (_retrievedPhotoCounter) + _photoPosition.x + ".png", bytes);

        using (StreamWriter writer = new StreamWriter(dirpath+file_name, true))
        {
            writer.WriteLine(_photoPosition.x);
        }
    ...
    private GameObject GenerateGameObject(Vector3 position, Quaternion rotation) 
    {
        ...
        //NEW
        /*
        string filePath = Application.persistentDataPath + "/SavedImage/" + "Image-";

        string temp_filePath = Application.persistentDataPath + "/SavedImage/" + "Image-" + _retrievedPhotoCounter;
        */
        //constant part of the path
        /*
        for (int i = 0; i < _file_names.Count - 1; i++)
        {
            if(_file_names[i].Contains(temp_filePath))
            {
                temp_filePath = _file_names[i];
            }
        }
        */
